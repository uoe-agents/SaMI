...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        policy_observation_space = deepcopy(self.observation_space)
        if self.causal_keys is not None:
            for key in self.causal_keys:
                assert key in policy_observation_space.spaces, \
                    f'causal must in observation get causal \'{key}\', observation keys {list(self.observation_space.keys())}'
                policy_observation_space.spaces.pop(key)
    ...
    def load_replay_buffer(
        ...
        assert isinstance(self.replay_buffer, DictReplayBuffer), "The replay buffer must inherit from ReplayBuffer class"
    ...
    def _store_transition(
        ...
        if 'action' in self._last_obs:
            self._last_obs.pop('action')
        ...
        _next_obs = OrderedDict()
        _next_obs['action'] = (buffer_action * (1-np.stack((dones,), axis = -1))).astype(np.float32)
        for key in next_obs:
            _next_obs[key] = next_obs[key].astype(np.float32)
        _next_obs['hidden_h'] = self._last_original_obs['hidden_h']
        _next_obs['hidden_c'] = self._last_original_obs['hidden_c']
        causal, hidden_h, hidden_c = self.policy.rnn_encoder_predict(_next_obs)
        next_obs['causal'] = causal.astype(np.float32)
        next_obs['hidden_h'] = hidden_h * (1-np.stack((dones,), axis = -1)).astype(np.float32)
        next_obs['hidden_c'] = hidden_c * (1-np.stack((dones,), axis = -1)).astype(np.float32)
        ...
        # update _last_obs
        _last_obs = OrderedDict()
        for key in new_obs:
            _last_obs[key] = new_obs[key]
        _last_obs['causal'] = next_obs['causal']
        _last_obs['hidden_h'] = next_obs['hidden_h']
        _last_obs['hidden_c'] = next_obs['hidden_c']

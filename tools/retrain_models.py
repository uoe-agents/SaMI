...
while len(need_to_run) > 0:
    ...
    while left_run_num > 0 and len(need_to_run) > 0 and gpu_num[gpu_id] > 0:
        test_log_file = os.path.join(need_to_run[0], 'log-test.txt')
        model_paths = os.path.join(need_to_run[0], 'model*.zip')
        ...
        if os.path.exists(test_log_file):
            with open(test_log_file,'r',encoding='utf-8') as f:
                ...
                # if data.strip() == '' :
                os.system(f'CUDA_VISIBLE_DEVICES={gpu_id} nohup python main.py --config_path {need_to_run[0]} --use_continue_train &')
                gpu_id += 1
                gpu_id %= _gpu_num
                gpu_num[gpu_id] -= 1
                need_to_run = need_to_run[1:]
